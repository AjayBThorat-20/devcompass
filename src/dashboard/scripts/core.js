...
(function() {
  ...
  window.graphPagination = {
    currentPage: 1,
    pageSize: 500
  };

  function init() {
    console.log('🚀 DevCompass Dashboard v3.2.6 initializing...');
    ...
    const totalNodes = window.graphData.nodes.length;
    const totalLinks = window.graphData.links?.length || 0;

    console.log(`📊 Loaded ${totalNodes} nodes, ${totalLinks} links`);

    if (totalNodes > window.graphPagination.pageSize) {
      console.log(`📄 Pagination enabled (${window.graphPagination.pageSize} nodes/page)`);
    }
  ...
  window.DevCompass = {
    ...
    resetPagination: function() {
      window.graphPagination.currentPage = 1;
    }
  };
