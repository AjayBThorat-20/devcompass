// src/features/ai/ai.database.js
...
class AIDatabase {
  ...
  addProvider(provider, apiKey, model, baseUrl = null) {
    return this.db.prepare('INSERT INTO llm_providers (provider, api_key, model, base_url) VALUES (?, ?, ?, ?)').run(provider, apiKey, model, baseUrl);
  }
  ...
  saveConversation(sessionId, providerId, command, commandOutput, userPrompt, aiResponse, tokensUsed, cost) {
    return this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sessionId, providerId, command, commandOutput, userPrompt, aiResponse, tokensUsed, cost);
  }

  getConversationHistory(sessionId, limit = 10) {
    return this.db.prepare('SELECT * FROM ai_conversations WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?').all(sessionId, limit);
  }
